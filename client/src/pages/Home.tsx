...
import { AbilityKind, FruitGame, GameSnapshot } from "@/game/gameEngine";
...
import { activateStoryMission, completeStoryMission, purchaseStoryPage, readStoryProgress, saveStoryProgress, STORY_MISSIONS, STORY_PAGE_PRICES, STORY_STORAGE_KEY, StoryProgress } from "@/game/story";
import { Volume2, VolumeX, Pause, Play, RotateCcw, HelpCircle, Settings2, ChevronLeft, Images, Shirt, BookOpen, LockKeyhole, Check } from "lucide-react";
...
function StoryPage({ progress, goldenCards, onActivate, onPurchase, onClose }: { progress: StoryProgress; goldenCards: number; onActivate: (chapter: number) => void; onPurchase: (chapter: number) => void; onClose: () => void }) {
  const [openChapter, setOpenChapter] = useState<number | null>(null);
  return <section className="story-page">
    <button className="back-link" onClick={onClose}><ChevronLeft size={17} /> Voltar</button>
    <div className="story-heading"><div><p className="eyebrow">UMA AVENTURA EM CAPÍTULOS</p><h2>História <em>Luli</em></h2><p>Ative as missões em ordem, complete cada desafio e desbloqueie novas páginas da história.</p></div><div className="story-progress-badge"><BookOpen size={18} /><strong>{progress.completed.filter(Boolean).length}/{STORY_MISSIONS.length}</strong><span>CAPÍTULOS</span></div></div>
    <div className="story-mission-strip"><span className="story-strip-label">MISSÕES DA HISTÓRIA</span>{STORY_MISSIONS.map((mission) => { const done = progress.completed[mission.id - 1]; const unlocked = mission.id <= progress.unlockedChapter; const active = progress.activeChapter === mission.id; return <div className={`story-mission-chip ${done ? "is-done" : ""} ${active ? "is-active" : ""} ${!unlocked ? "is-locked" : ""}`} key={mission.id}><b>{mission.id}</b><span>{done ? "CONCLUÍDA" : active ? "ATIVA" : unlocked ? "DISPONÍVEL" : "BLOQUEADA"}</span></div>; })}</div>
    <div className="story-grid">{STORY_MISSIONS.map((mission) => { const done = progress.completed[mission.id - 1]; const purchased = progress.purchased[mission.id - 1]; const missionAvailable = mission.id === progress.unlockedChapter && !done; const pageUnlocked = done || purchased; const active = progress.activeChapter === mission.id; return <article className={`story-card ${done ? "is-done" : purchased ? "is-purchased" : "is-locked"}`} key={mission.id}>
      <button className="story-page-button" disabled={!pageUnlocked} onClick={() => setOpenChapter(mission.id)}><img src={asset(mission.image)} alt={pageUnlocked ? `Capítulo ${mission.id}` : "Capítulo bloqueado"} />{!pageUnlocked && <span className="story-lock"><LockKeyhole size={27} /></span>}{done && <span className="story-check"><Check size={18} /></span>}<strong>CAPÍTULO {mission.id}</strong></button>
      <div className="story-card-info"><h3>{mission.title}</h3><p>{mission.description}</p>{purchased ? <span className="story-status purchased">PÁGINA COMPRADA · QUEST CONCLUÍDA</span> : done ? <span className="story-status done">PÁGINA DESBLOQUEADA PELA QUEST</span> : <><button className="story-buy" onClick={() => onPurchase(mission.id)} disabled={goldenCards < STORY_PAGE_PRICES[mission.id - 1]}>COMPRAR POR {STORY_PAGE_PRICES[mission.id - 1]} CARTAS</button>{active ? <span className="story-status active">QUEST ATIVA</span> : missionAvailable ? <button className="story-activate" onClick={() => onActivate(mission.id)}>ATIVAR QUEST</button> : <span className="story-status">CONCLUA O CAPÍTULO ANTERIOR</span>}</>}</div>
    </article>; })}</div>
    {openChapter && progress.completed[openChapter - 1] && <div className="story-viewer" onClick={() => setOpenChapter(null)}><div className="story-viewer-card" onClick={(event) => event.stopPropagation()}><button className="draw-close" onClick={() => setOpenChapter(null)} aria-label="Fechar">×</button><img src={asset(STORY_MISSIONS[openChapter - 1].image)} alt={`Capítulo ${openChapter}`} /><p>CAPÍTULO {openChapter} · clique fora para fechar</p></div></div>}
  </section>;
}
...
export default function Home() {
  ...
  const [screen, setScreen] = useState<"menu" | "loading" | "game" | "help" | "settings" | "album" | "wardrobe" | "story">("menu"); const [state, setState] = useState<GameSnapshot | null>(null); const [sound, setSound] = useState(() => localStorage.getItem("luli-sound") !== "off"); const [musicVolume, setMusicVolume] = useState(() => Number(localStorage.getItem("luli-music-volume") ?? 0.35)); const [effectsVolume, setEffectsVolume] = useState(() => Number(localStorage.getItem("luli-effects-volume") ?? 0.7)); const [effects, setEffects] = useState(true); const [album, setAlbum] = useState<AlbumProgress>(() => readAlbum()); const [missions, setMissions] = useState<MissionState>(() => readMissionState()); const [missionsOpen, setMissionsOpen] = useState(false); const [story, setStory] = useState<StoryProgress>(() => readStoryProgress());
  ...
  const luckyThresholdNoticeShown = useRef(false);
  ...
  useEffect(() => {
    ...
    const welcome = new Audio(asset("Lulihome.mp3"));
    welcome.loop = false;
    welcome.volume = musicVolume;
    welcomeAudioRef.current = welcome;
    return () => {
      music.pause(); music.currentTime = 0;
      welcome.pause(); welcome.currentTime = 0;
      musicRef.current = null; welcomeAudioRef.current = null;
    };
  }, []);

  const playWelcomeAudio = () => {
    if (welcomePlayedRef.current || !sound || screen !== "menu") return;
    const welcome = welcomeAudioRef.current;
    if (!welcome) return;
    welcomePlayedRef.current = true;
    welcome.currentTime = 0;
    welcome.play().catch(() => { welcomePlayedRef.current = false; });
  };

  useEffect(() => {
    if (screen !== "menu") return;
    playWelcomeAudio();
    const unlockWelcomeAudio = () => playWelcomeAudio();
    window.addEventListener("pointerdown", unlockWelcomeAudio, { once: true });
    window.addEventListener("keydown", unlockWelcomeAudio, { once: true });
    return () => {
  ...
  useEffect(() => {
    ...
    if (welcome) welcome.volume = musicVolume;
    localStorage.setItem("luli-music-volume", String(musicVolume));
  }, [musicVolume]);
  ...
  useEffect(() => {
    ...
    if (screen === "game") music.play().catch(() => {});
    localStorage.setItem("luli-sound", "on");
  }, [sound, screen]);

  useEffect(() => {
    if (screen !== "menu") welcomeAudioRef.current?.pause();
  }, [screen]);
  ...
  const activateStory = (chapter: number) => setStory((current) => activateStoryMission(current, chapter));
  const purchaseStory = (chapter: number) => {
    const price = STORY_PAGE_PRICES[chapter - 1];
    if (!price || album.goldenCards < price || story.completed[chapter - 1] || story.purchased[chapter - 1]) return;
    const next = purchaseStoryPage(story, chapter);
    setStory(next);
    saveAlbum({ ...album, goldenCards: album.goldenCards - price });
    announce(`Página ${chapter} comprada. Quest concluída e próxima missão liberada!`);
  };
  const checkStoryMission = () => {
    const chapter = story.activeChapter;
    if (!chapter || story.completed[chapter - 1]) return;
    const bodies = gameRef.current?.getBodies() ?? [];
    const countLevel = (level: number) => bodies.filter((body) => body.fruitLevel === level).length;
    const allFruitSkinsEquipped = FRUITS.every((fruit) => Boolean(equippedSkins[fruit.id]));
    const complete = chapter === 1 || chapter === 6 ? countLevel(10) >= 2
      : chapter === 2 || chapter === 7 ? countLevel(10) >= 3
      : chapter === 3 ? (state?.score ?? 0) >= 80000
      : chapter === 4 ? (state?.score ?? 0) >= 1500 && allFruitSkinsEquipped
      : chapter === 5 ? album.owned.length >= STICKERS.length
      : chapter === 8 ? countLevel(5) >= 5
      : chapter === 9 ? (state?.score ?? 0) >= 100000
      : false;
    if (!complete) return;
    const next = completeStoryMission(story, chapter);
    if (next === story) return;
    setStory(next);
    saveAlbum({ ...album, goldenCards: album.goldenCards + 1 });
    announce(`Capítulo ${chapter} desbloqueado! +1 Carta Dourada.`);
  };
  ...
  useEffect(() => {
    if (!state) return;
    if (state.score >= 50000 && !luckyThresholdNoticeShown.current) {
      luckyThresholdNoticeShown.current = true;
      announce("Sorte grande pode aparecer a qualquer jogada!");
    }
  ...
  useEffect(() => { checkStoryMission(); }, [state, album.owned.length, equippedSkins, story.activeChapter]);
  ...
  useEffect(() => { const game = new FruitGame(); gameRef.current = game; const canvas = canvasRef.current; if (!canvas) return;
    ...
    const draw = () => { game.update(); const ctx = canvas.getContext("2d"); if (ctx) { ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = "high"; const dpr = canvas.width / canvas.getBoundingClientRect().width; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); const w = canvas.clientWidth, h = canvas.clientHeight; const snapshot = stateRef.current; ctx.clearRect(0, 0, w, h);
      ...
      if (screen === "game" && !snapshot?.over && !snapshot?.paused && !snapshot?.activeAbility) { const next = snapshot?.next; if (next) { ctx.save(); ctx.globalAlpha = .22; ctx.setLineDash([4, 8]); ctx.strokeStyle = "#174A4B"; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(previewX.current, Math.max(58 + next.radius, previewY.current - 8)); ctx.lineTo(previewX.current, h - 9); ctx.stroke(); ctx.restore(); ctx.globalAlpha = .8; drawFruitImage(ctx, previewX.current, previewY.current, next.radius, next, 0, equippedSkinsRef.current[next.id as SkinId]); ctx.globalAlpha = 1; } }
    } frameRef.current = requestAnimationFrame(draw); }; frameRef.current = requestAnimationFrame(draw);
    const ability = game.onAbility((x, y, text) => { setFlash({ x, y, text }); window.setTimeout(() => setFlash(null), 900); });
    return () => { cancelAnimationFrame(frameRef.current); window.removeEventListener("resize", resize); unsubscribe(); unmerge(); ability(); };
  }, [screen]);

  useEffect(() => { gameRef.current?.setAudio(sound); gameRef.current?.setEffectsVolume(effectsVolume); localStorage.setItem("luli-sound", sound ? "on" : "off"); localStorage.setItem("luli-effects-volume", String(effectsVolume)); }, [sound, effectsVolume]);
  const start = () => {
    luckyThresholdNoticeShown.current = false;
  ...
  const restartSave = () => { if (!window.confirm("Apagar todo o progresso do álbum, missões, skins e recorde?")) return; localStorage.removeItem("luli-album"); localStorage.removeItem("luli-best"); localStorage.removeItem(MISSION_STORAGE_KEY); localStorage.removeItem("luli-owned-skins"); localStorage.removeItem("luli-equipped-skins"); localStorage.removeItem(STORY_STORAGE_KEY); setAlbum(EMPTY_ALBUM); setOwnedSkins([]); setEquippedSkins({}); const freshMissions = createNextMissionState(missions); saveMissions(freshMissions); setMissionsOpen(false); setStory({ activeChapter: null, unlockedChapter: 1, completed: [], purchased: [] }); gameRef.current?.reset(); setState(null); announce("Novo save criado."); };
  ...
  const drop = (event: React.PointerEvent<HTMLCanvasElement>) => {
    const rect = event.currentTarget.getBoundingClientRect();
    const x = event.clientX - rect.left;
    const y = event.clientY - rect.top;
    if (stateRef.current?.activeAbility) { gameRef.current?.useAbilityAt(x, y); return; }
    pointerPosition(event); gameRef.current?.drop(previewX.current);
  };
  const chooseAbility = (kind: AbilityKind) => gameRef.current?.chooseAbility(kind);
  ...
  return <main className="app-shell"><div className="paper-speckles" />
      ...
      <section
        ...
        style={{
          backgroundImage: `url(${asset(
            album.owned.length >= STICKERS.length
              ? "luli-merge-loading-late.png"
              : "luli-merge-loading-day.png"
          )})`,
        }}
        aria-label="A carregar o jogo"
      >
        <div className="loading-progress">
          <div className="loading-progress-bar" />
        </div>
        <span>A PREPARAR O JOGO</span>
      </section>
    )}
    <header className={`topbar ${screen === "game" ? "game-topbar" : ""}`}><button className="brand" onClick={() => setScreen("menu")}><img src={LOGO} alt="" /><span className="brand-wordmark">LULI <b>FRUIT</b><i aria-hidden="true" /></span></button><div className="topbar-note">A tua pausa mais saborosa</div><button className="sound-button" aria-label={sound ? "Desligar som" : "Ligar som"} onClick={() => setSound(!sound)}>{sound ? <Volume2 size={19} /> : <VolumeX size={19} />}</button></header>
      {screen === "menu" && <section className="menu-layout" onPointerDown={playWelcomeAudio}><div className="menu-copy"><p className="eyebrow">PEQUENAS FRUTAS. GRANDES FUSÕES.</p><h1>Junta tudo<br /><em>com sabor.</em></h1><p className="intro">Deixa cair, combina frutas iguais e vê até onde consegues chegar antes de encher o frasco.</p><button className="primary-button" onClick={start}>JOGAR AGORA <span>→</span></button><div className="menu-links"><button onClick={() => setScreen("help")}><HelpCircle size={17} /> Como jogar</button><button onClick={() => setScreen("album")}><Images size={17} /> Álbum</button><button onClick={() => setScreen("story")}><BookOpen size={17} /> História</button><button onClick={() => setScreen("wardrobe")}><Shirt size={17} /> Armário</button><button onClick={() => setScreen("settings")}><Settings2 size={17} /> Definições</button></div></div><div className="menu-art"><div className="sticker sticker-top">NOVIDADE<br /><strong>+10</strong> frutas</div><img className="menu-logo" src={LOGO} alt="Luli Fruit" /><div className="menu-score-card"><span>RECORDE ATUAL</span><strong>{state?.best || 0}</strong><i>continua a subir</i></div></div></section>}
    {screen === "game" && <section className="game-layout"><div className="game-hud"><div className="hud-stats"><strong>{(state?.score || 0).toLocaleString("pt-PT")}</strong><span>🏆 {(state?.best || 0).toLocaleString("pt-PT")}</span><span>🎴 {Math.max(0, 100000 - album.points).toLocaleString("pt-PT")}</span></div><button className="missions-button" onClick={() => { missionOpenedFromRunning.current = !state?.paused; if (!state?.paused) gameRef.current?.togglePause(); setMissionsOpen(true); }}><span>✦</span><b>MISSÕES</b><small>{missions.progress.filter((entry) => entry.completed).length}/{missions.progress.length}</small></button><div className="hud-next"><button className="pause-icon" onClick={() => gameRef.current?.togglePause()} aria-label={state?.paused ? "Continuar" : "Pausar"}>{state?.paused ? <Play size={16} /> : <Pause size={16} />}</button>{state?.next && <FruitBadge fruit={state.next} small skinImage={equippedSkins[state.next.id as SkinId]} />}</div></div><div className="board-wrap">{rewardNotice && <div className="reward-notice">{rewardNotice}</div>}<div className="board-heading"><span className="stamp-label">RONDA LIVRE</span><small>toque para posicionar · toque novamente para largar</small></div><div className="board"><canvas ref={canvasRef} onPointerMove={pointerPosition} onPointerDown={pointerStart} onPointerUp={drop} onKeyDown={key} tabIndex={0} aria-label="Tabuleiro de LULI FRUIT" />{flash && effects && <div className="merge-flash" style={{ left: flash.x, top: flash.y }}>{flash.text}</div>}{state?.activeAbility && <div className="ability-active-hint">{state.activeAbility === "prism-orb" ? "ORBE DE PRISMA" : "BOMBA FRUTÍFERA"}: clique em uma fruta</div>}{state?.abilityOffer && <div className="modal-cover ability-offer"><div className="modal-card ability-modal"><span className="modal-kicker">SORTE GRANDE!</span><h2>Escolha uma habilidade</h2><p>Use-a clicando em uma fruta do tabuleiro. Ela só pode ser usada uma vez nesta partida.</p><div className="ability-options"><button onClick={() => chooseAbility("prism-orb")}><img src={asset("habilidade-orbe.png")} alt="Orbe de Prisma" /><strong>ORBE DE PRISMA</strong><small>Avança todas as frutas do tipo e dá 1.000 pontos por fruta.</small></button><button onClick={() => chooseAbility("fruit-bomb")}><img src={asset("habilidade-bomba.png")} alt="Bomba Frutífera" /><strong>BOMBA FRUTÍFERA</strong><small>Destrói todas as frutas do tipo e dá 250 pontos por fruta.</small></button></div></div></div>}{state?.paused && !missionsOpen && !state?.abilityOffer && <div className="modal-cover"><div className="modal-card"><span className="modal-kicker">A FRUTEIRA EM PAUSA</span><h2>Respira.<br /><em>Já voltamos.</em></h2><button className="primary-button" onClick={() => gameRef.current?.togglePause()}>CONTINUAR <Play size={16} /></button><button className="text-button" onClick={start}><RotateCcw size={15} /> Recomeçar</button></div></div>}{state?.over && <div className="modal-cover"><div className="modal-card"><span className="modal-kicker">A FRUTEIRA FICOU CHEIA</span><h2>Boa colheita.<br /><em>Outra vez?</em></h2><div className="final-score"><span>PONTUAÇÃO</span><b>{state.score}</b></div><button className="primary-button" onClick={start}>JOGAR NOVAMENTE <RotateCcw size={16} /></button><button className="text-button" onClick={() => setScreen("menu")}><ChevronLeft size={15} /> Menu principal</button></div></div>}</div></div>{missionsOpen && <><div className="mission-backdrop" onClick={() => { setMissionsOpen(false); if (missionOpenedFromRunning.current) gameRef.current?.togglePause(); }} /><MissionDrawer missions={missions} onClose={() => { setMissionsOpen(false); if (missionOpenedFromRunning.current) gameRef.current?.togglePause(); }} /></>}</section>}
    {screen === "story" && <StoryPage progress={story} goldenCards={album.goldenCards} onActivate={activateStory} onPurchase={purchaseStory} onClose={() => setScreen("menu")} />}
    ...
    {state?.paused && !state?.abilityOffer && <div className="pause-menu-panel"><div className="pause-menu-mark"><Pause size={18} /></div><span className="modal-kicker">A FRUTEIRA EM PAUSA</span><h2>Respira.<br /><em>Já voltamos.</em></h2><p>A tua colheita está guardada nesta partida.</p><div className="pause-audio-controls"><label><span>♫ Música</span><strong>{sound ? "LIGADA" : "DESLIGADA"}</strong><button type="button" onClick={() => setSound(!sound)}>{sound ? "DESLIGAR" : "LIGAR"}</button></label><label><span>Volume da música</span><input type="range" min="0" max="1" step="0.01" value={musicVolume} onChange={(event) => setMusicVolume(Number(event.target.value))} /></label><label><span>Volume dos efeitos</span><input type="range" min="0" max="1" step="0.01" value={effectsVolume} onChange={(event) => setEffectsVolume(Number(event.target.value))} /></label></div><button className="pause-menu-close" onClick={() => gameRef.current?.togglePause()}><Play size={17} /> FECHAR</button><button className="pause-menu-home" onClick={leaveGame}><ChevronLeft size={17} /> VOLTAR AO INÍCIO</button><button className="pause-menu-restart" onClick={start}><RotateCcw size={16} /> RECOMEÇAR PARTIDA</button></div>}
  </main>;
}
