...
function FruitBadge({ fruit, small = false }: { fruit: FruitDefinition; small?: boolean }) {
  ...
  return <img className={`fruit-badge ${small ? "fruit-badge--small" : ""}`} src={displayedFruit.image} alt={displayedFruit.name} />;
}
...
export default function Home() {
  ...
  const restartSave = () => { if (!window.confirm("Apagar todo o progresso do álbum, missões e recorde?")) return; localStorage.removeItem("luli-album"); localStorage.removeItem("luli-best"); localStorage.removeItem(MISSION_STORAGE_KEY); setAlbum(EMPTY_ALBUM); const freshMissions = createNextMissionState(missions); saveMissions(freshMissions); setMissionsOpen(false); gameRef.current?.reset(); setState(null); announce("Novo save criado."); };
  ...
  return <main className="app-shell"><div className="paper-speckles" />
    ...
    <footer className={`footer-note ${screen === "game" ? "game-footer" : ""}`}>LULI FRUIT <span>feito para jogar mais uma</span></footer>
